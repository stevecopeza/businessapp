# BusinessApp — Phase Roadmap (Conceptual)

Phase 1: Quoting clarity (Complete)
- Quote creation, editing, and lifecycle management.
- PDF generation and email sending.
- Customer acceptance/rejection flow.

Phase 2: Light job tracking (Complete)
- [x] Job creation from quotes.
- [x] Direct job creation.
- [x] Job item management (Add/Edit/Remove items).
- [x] Job status workflow (Active -> Completed).
- [x] Basic time/material tracking.

Phase 3: Simple financial outputs (Complete)
- [x] Invoicing based on completed jobs.
- [x] Basic revenue reporting.

Phase 4: Client Communication & Public Views (In Progress)
- [x] Public-facing Quote/Invoice views (HTML + Print CSS).
- [x] Secure token generation.
- [ ] Email notifications via wp_mail.
- [ ] "View as Client" button for Admins.

Phase 5: Payments & Integrations (Mostly Complete)
- [x] Stripe Payment Gateway integration.
- [x] "Pay Now" button on public invoices.
- [x] Payment entity and database table.
- [x] Manual payment recording (cash/check).
- [x] Invoice balance tracking and status automation.

Each phase builds on the last.
